...
async def health_check():
    """Detailed health check endpoint to verify API is working."""
...
class AutoConfirmUserRequest(BaseModel):
    user_id: str

@app.post("/api/auth/auto-confirm-user")
async def auto_confirm_user(request: AutoConfirmUserRequest):
    """Auto-confirm a user account to bypass email verification."""
    try:
        ...
        if response.user:
            logger.info(f"User {request.user_id} auto-confirmed successfully")
            return {"status": "success", "message": "User confirmed successfully"}
        else:
            logger.error(f"Failed to auto-confirm user {request.user_id}")
            raise HTTPException(status_code=400, detail="Failed to confirm user")
            
    except Exception as e:
        logger.error(f"Error auto-confirming user {request.user_id}: {e}")

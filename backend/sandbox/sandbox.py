...
# Initialize Daytona only if API key is provided
daytona = None
if config.DAYTONA_API_KEY:
    ...
    logger.warning("No Daytona API key found - sandbox functionality will be limited")

async def get_or_start_sandbox(sandbox_id: str):
    """Retrieve a sandbox by ID, check its state, and start it if needed."""
    
    if not daytona:
        logger.error("Daytona client not initialized - cannot manage sandboxes")
        raise Exception("Sandbox functionality requires Daytona API key")
...
def create_sandbox(password: str, project_id: str = None):
    ...
    if not daytona:
        logger.error("Daytona client not initialized - cannot create sandboxes")
        raise Exception("Sandbox functionality requires Daytona API key")
...
async def delete_sandbox(sandbox_id: str):
    ...
    if not daytona:
        logger.error("Daytona client not initialized - cannot delete sandboxes")
        raise Exception("Sandbox functionality requires Daytona API key")

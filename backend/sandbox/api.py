...
async def verify_sandbox_access(client, sandbox_id: str, user_id: Optional[str] = None):
    ...
    project_result = client.table('projects').select('*').filter('sandbox->>id', 'eq', sandbox_id).execute()
    ...
    if account_id:
        account_user_result = client.schema('public').from_('account_user').select('account_role').eq('user_id', user_id).eq('account_id', account_id).execute()
...
async def get_sandbox_by_id_safely(client, sandbox_id: str):
    ...
    """
    # Find the project that owns this sandbox
    project_result = client.table('projects').select('project_id').filter('sandbox->>id', 'eq', sandbox_id).execute()

...
class UpdateAgentTool(Tool):
    ...
    async def update_agent(
        ...
        try:
            ...
            result = client.table('agents').update(update_data).eq('agent_id', self.agent_id).execute()
    ...
    async def get_current_agent_config(self) -> ToolResult:
        ...
        try:
            ...
            result = client.table('agents').select('*').eq('agent_id', self.agent_id).execute()
        ...
        try:
            async with httpx.AsyncClient() as client:
                headers = {
                    ...
                    "User-Agent": "Neo-MCP-Integration/1.0"
                }
    ...
    async def configure_mcp_server(
        ...
        try:
            ...
            result = client.table('agents').select('configured_mcps').eq('agent_id', self.agent_id).execute()

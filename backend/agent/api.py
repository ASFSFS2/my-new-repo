...
#     client = await db.client
#     running_agent_runs = client.table('agent_runs').select('id').eq("status", "running").execute()
...
async def check_for_active_project_agent_run(client, project_id: str):
    ...
    """
    project_threads = client.table('threads').select('thread_id').eq('project_id', project_id).execute()
    project_thread_ids = [t['thread_id'] for t in project_threads.data]

    if project_thread_ids:
        active_runs = client.table('agent_runs').select('id').in_('thread_id', project_thread_ids).eq('status', 'running').execute()
...
async def get_agent_run_with_access_check(client, agent_run_id: str, user_id: str):
    ...
    agent_run = client.table('agent_runs').select('*').eq('id', agent_run_id).execute()
...
async def start_agent(
    ...
    await verify_thread_access(client, thread_id, user_id)
    thread_result = client.table('threads').select('project_id', 'account_id', 'agent_id', 'metadata').eq('thread_id', thread_id).execute()
    ...
    if effective_agent_id:
        agent_result = client.table('agents').select('*').eq('agent_id', effective_agent_id).eq('account_id', account_id).execute()
    ...
    if not agent_config:
        default_agent_result = client.table('agents').select('*').eq('account_id', account_id).eq('is_default', True).execute()
    ...
    if body.agent_id and body.agent_id != thread_agent_id and agent_config:
        try:
            client.table('threads').update({"agent_id": agent_config['agent_id']}).eq('thread_id', thread_id).execute()
    ...
    try:
        ...
        project_result = client.table('projects').select('*').eq('project_id', project_id).execute()
    ...
    agent_run = client.table('agent_runs').insert({
...
async def get_agent_runs(thread_id: str, user_id: str = Depends(get_current_user_id_from_jwt)):
    ...
    client = await db.client
    await verify_thread_access(client, thread_id, user_id)
    agent_runs = client.table('agent_runs').select('*').eq("thread_id", thread_id).order('created_at', desc=True).execute()
...
async def get_thread_agent(thread_id: str, user_id: str = Depends(get_current_user_id_from_jwt)):
    ...
    try:
        ...
        await verify_thread_access(client, thread_id, user_id)
        thread_result = client.table('threads').select('agent_id', 'account_id').eq('thread_id', thread_id).execute()
        ...
        if not effective_agent_id:
            ...
            default_agent_result = client.table('agents').select('agent_id').eq('account_id', account_id).eq('is_default', True).execute()
            if default_agent_result.data:
...
async def stream_agent_run(
    ...
    async def stream_generator():
        ...
        try:
            ...
            run_status = client.table('agent_runs').select('status').eq("id", agent_run_id).maybe_single().execute()
...
async def generate_and_update_project_name(project_id: str, prompt: str):
    ...
    try:
        ...
        if generated_name:
            update_result = client.table('projects').update({"name": generated_name}).eq("project_id", project_id).execute()
...
async def initiate_agent_with_files(
    ...
    agent_config = None
    if agent_id:
        agent_result = client.table('agents').select('*').eq('agent_id', agent_id).eq('account_id', account_id).execute()
        if not agent_result.data:
            raise HTTPException(status_code=404, detail="Agent not found or access denied")
        agent_config = agent_result.data[0]
    ...
    try:
        ...
        # 2. Create Sandbox
        try:
            from sandbox.hybrid_sandbox import create_sandbox
            sandbox_result = create_sandbox(password=str(uuid.uuid4()), project_id=project_id)
            
            if sandbox_result and 'id' in sandbox_result:
                sandbox_id = sandbox_result['id']
                sandbox_pass = sandbox_result.get('pass', str(uuid.uuid4()))
                vnc_url = sandbox_result.get('vnc_preview', 'http://localhost:6080')
                website_url = sandbox_result.get('sandbox_url', 'http://localhost:8080')
                token = sandbox_result.get('token', 'test-token')
                logger.info(f"Created sandbox: {sandbox_id}")
            else:
                # Fallback to mock values if sandbox creation fails
                sandbox_id = "test-sandbox-id"
                sandbox_pass = str(uuid.uuid4())
                vnc_url = "http://localhost:6080"
                website_url = "http://localhost:8080"
                token = "test-token"
                logger.warning(f"Sandbox creation failed, using mock values")
        except Exception as e:
        ...
        update_result = client.table('projects').update({
            'sandbox': {
        ...
        thread = client.table('threads').insert(thread_data).execute()
...
async def get_agent(agent_id: str, user_id: str = Depends(get_current_user_id_from_jwt)):
    ...
    try:
        ...
        agent = client.table('agents').select('*').eq("agent_id", agent_id).execute()
...
async def create_agent(
    ...
    try:
        ...
        if agent_data.is_default:
            client.table('agents').update({"is_default": False}).eq("account_id", user_id).eq("is_default", True).execute()
        ...
        new_agent = client.table('agents').insert(insert_data).execute()
...
async def update_agent(
    ...
    try:
        ...
        existing_agent = client.table('agents').select('*').eq("agent_id", agent_id).eq("account_id", user_id).maybe_single().execute()
        ...
        if agent_data.is_default is not None:
            ...
            if agent_data.is_default:
                client.table('agents').update({"is_default": False}).eq("account_id", user_id).eq("is_default", True).neq("agent_id", agent_id).execute()
        ...
        if not update_data:
            ...
            # Update the agent
            update_result = client.table('agents').update(update_data).eq("agent_id", agent_id).eq("account_id", user_id).execute()
            
            if not update_result.data:
                raise HTTPException(status_code=500, detail="Failed to update agent")
            
            # Fetch the updated agent data
            updated_agent = client.table('agents').select('*').eq("agent_id", agent_id).eq("account_id", user_id).maybe_single().execute()
...
async def delete_agent(agent_id: str, user_id: str = Depends(get_current_user_id_from_jwt)):
    ...
    try:
        ...
        agent_result = client.table('agents').select('*').eq('agent_id', agent_id).execute()
        if not agent_result.data:
            raise HTTPException(status_code=404, detail="Agent not found")
        ...
        # Delete the agent
        client.table('agents').delete().eq('agent_id', agent_id).execute()
...
async def publish_agent_to_marketplace(
    ...
    try:
        ...
        client.table('agents').update(update_data).eq('agent_id', agent_id).execute()
...
async def get_user_agent_library(user_id: str = Depends(get_current_user_id_from_jwt)):
    ...
    try:
        result = client.table('user_agent_library').select("""
            *,
...
async def get_agent_builder_chat_history(
    ...
    try:
        # First verify the agent exists and belongs to the user
        agent_result = client.table('agents').select('*').eq('agent_id', agent_id).eq('account_id', user_id).execute()
        if not agent_result.data:
            raise HTTPException(status_code=404, detail="Agent not found or access denied")
        
        # Get all threads for this user with metadata field included
        threads_result = client.table('threads').select('thread_id, created_at, metadata').eq('account_id', user_id).order('created_at', desc=True).execute()
        ...
        messages_result = client.table('messages').select('*').eq('thread_id', latest_thread_id).neq('type', 'status').neq('type', 'summary').order('created_at', desc=False).execute()

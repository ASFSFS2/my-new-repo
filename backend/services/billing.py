"""
NEO Billing Service - Simplified version without limitations.
All users have unlimited access to NEO features.
"""
...
async def get_stripe_customer_id(client, user_id: str) -> Optional[str]:
    ...
    result = await client.schema('public').from_('billing_customers') \
        .select('id') \
...
async def create_stripe_customer(client, user_id: str, email: str) -> str:
    ...
    await client.schema('public').from_('billing_customers').insert({
        'id': customer.id,
        'account_id': user_id,
        'email': email,
        'provider': 'stripe'
...
async def calculate_monthly_usage(client, user_id: str) -> float:
    ...
    threads_result = client.table('threads') \
        .select('thread_id') \
        .eq('account_id', user_id) \
        .execute()
        ...
        if existing_subscription:
            ...
            try:
                ...
                if is_upgrade:
                    ...
                    await client.schema('public').from_('billing_customers').update(
                        {'active': True}

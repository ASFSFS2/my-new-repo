...
import { simpleSignIn, simpleSignUp } from './simple-actions';
...
function LoginContent() {
  ...
  const handleSignIn = async (prevState: any, formData: FormData) => {
    ...
    const result = await simpleSignIn(prevState, formData);
  ...
  const handleSignUp = async (prevState: any, formData: FormData) => {
    ...
    const result = await simpleSignUp(prevState, formData);
    ...
    <main className="flex flex-col items-center justify-center min-h-screen w-full">
      ...
      <Dialog open={forgotPasswordOpen} onOpenChange={setForgotPasswordOpen}>
        <DialogContent className="sm:max-w-md rounded-xl bg-[#F3F4F6] dark:bg-[#17171A] border border-border [&>button]:hidden">
          ...
          <form onSubmit={handleForgotPassword} className="space-y-4 py-4" noValidate>
            <Input
